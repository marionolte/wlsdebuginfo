...
//    private static Log log = LogFactory.getLog(SessionChannelClient.class);
    ...
    public boolean setEnvironmentVariable(String name, String value) throws IOException {
        final String func=getFunc("setEnvironmentVariable(String name, String value)");
        printf(func,2,"Requesting environment variable to be set [" + name + "=" + value + "]");
    ...
    public boolean requestAgentForwarding() throws IOException {
        final String func=getFunc("requestAgentForwarding()");
        printf(func,3,"Requesting agent forwarding for the session");
    ...
    public boolean requestX11Forwarding(int display, String cookie) throws IOException {
        final String func=getFunc("requestX11Forwarding(int display, String cookie)");
        printf(func,2,"Requesting X11 forwarding for display " + display +
            " using cookie " + cookie);
    ...
    public void changeTerminalDimensions(PseudoTerminal term) throws IOException {
        final String func=getFunc("changeTerminalDimensions(PseudoTerminal term)");
        printf(func,2,"Changing terminal dimensions");
    ...
    public boolean executeCommand(String command) throws IOException {
        final String func=getFunc("executeCommand(String command)");
        printf(func,3,"Requesting command execution\nCommand is " + command);
    ...
    public boolean requestPseudoTerminal(String term, int cols, int rows, int width, int height, String terminalModes) throws IOException {
        final String func=getFunc("requestPseudoTerminal(String term, int cols, int rows, int width, int height, String terminalModes)");
        printf(func,3,"Requesting pseudo terminal");

        if (log.isDebugEnabled()) {
            printf(func,2,"Terminal Type is " + term);
            printf(func,2,"Columns=" + String.valueOf(cols));
            printf(func,2,"Rows=" + String.valueOf(rows));
            printf(func,2,"Width=" + String.valueOf(width));
            printf(func,2,"Height=" + String.valueOf(height));
        }
    ...
    public boolean startShell() throws IOException {
        final String func=getFunc("startShell");
        printf(func,2,"Requesting users shell");
    ...
    public boolean startSubsystem(String subsystem) throws IOException {
        final String func=getFunc("startSubsystem(String subsystem)");
        printf(func,3,"Starting " + subsystem + " subsystem");
    ...
    protected void onChannelClose() throws IOException {
        final String func=getFunc("onChannelClose()");
        ...
        if (exitCode != null) {
            printf(func,2,"Exit code " + exitCode.toString());
        }
    }
    ...
    protected void onChannelRequest(String requestType, boolean wantReply, byte[] requestData) throws IOException {
        final String func=getFunc("onChannelRequest(String requestType, boolean wantReply, byte[] requestData)");
        printf(func,3,"Channel Request received: " + requestType);

        if (requestType.equals("exit-status")) {
            exitCode = new Integer((int) ByteArrayReader.readInt(requestData, 0));
            //printf(func,2,"Exit code of " + exitCode.toString() + " received");
            printf(func,3,"Exit code of " + exitCode.toString() + " received");
            ...
            printf(func,2,"Exit signal " + signal + " received");
            printf(func,2,"Signal message: " + message);
            printf(func,2,"Core dumped: " + String.valueOf(coredump));
            ...
            printf(func,2,"Signal " + signal + " received");
