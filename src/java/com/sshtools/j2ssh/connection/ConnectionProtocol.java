...
//    private static Log log = LogFactory.getLog(ConnectionProtocol.class);
        ...
        //synchronized (activeChannels) {
            ...
            try {
                ...
                if (result.getMessageId() == SshMsgChannelOpenConfirmation.SSH_MSG_CHANNEL_OPEN_CONFIRMATION) {
                    ...
                    printf(func,2,"Initiating channel");
    ...
    protected synchronized void onStop() {
        final String func=getFunc();
        printf(func,4,"Closing all active channels");
	//	synchronized (activeChannels) {
		printf(func,3,"thread has "+activeChannels.values().size()+" active channels to stop");
        try {
            Channel channel;
            ...
            for (Iterator x = activeChannels.values().iterator(); x.hasNext();) {
                ...
                if (channel != null) {
                    if (log.isDebugEnabled()) {
                        printf(func,2,"Closing " + channel.getName() + " id=" +
                            String.valueOf(channel.getLocalChannelId()));
                    }
			...
			printf(func,1,"Unable to close all channels: "+t.getMessage(), (Exception) t);
        }
    ...
    public synchronized void sendChannelData(Channel channel, byte[] data)
        ...
        synchronized (channel.getState()) {
            if (log.isDebugEnabled()) {
                printf(func,2,"Sending " + String.valueOf(data.length) +
    ...
    public void sendChannelEOF(Channel channel) throws IOException {
        ...
        final String func=getFunc();
            ...
            printf(func,3,"Local computer has set channel " +
    ...
    public synchronized boolean sendChannelRequest(Channel channel,
        ...
        printf(func,3,"Sending " + requestType + " request for the " +
        ...
        if (wantReply) {
            ...
            printf(func,3,"Waiting for channel request reply");
            ...
            try {
              ...
              case SshMsgChannelSuccess.SSH_MSG_CHANNEL_SUCCESS: {
                printf(func,3,"Channel request succeeded");
                success = true;
              ...
              case SshMsgChannelFailure.SSH_MSG_CHANNEL_FAILURE: {
                printf(func,3,"Channel request failed");
                success = false;
    ...
    public void sendChannelWindowAdjust(Channel channel, long bytesToAdd)
        throws IOException {
        final String func=getFunc();
        printf(func,2,"Increasing window size by " + String.valueOf(bytesToAdd) +
            " bytes");
    ...
    public synchronized byte[] sendGlobalRequest(String requestName,
        ...
        if (wantReply) {
            ...
            printf(func,2,"Waiting for global request reply");
    ...
    protected void closeChannel(Channel channel) throws IOException {
        ...
        printf(func,3,"Local computer has closed channel " +
            String.valueOf(channel.getLocalChannelId()) + "[" +
            channel.getName() + "]");
        transport.sendMessage(msg, this);
    }
    ...
    protected void onGlobalRequest(String requestName, boolean wantReply,
        ...
        final String func=getFunc();
        printf(func,2,"Processing " + requestName + " global request");
    ...
    protected void onMessageReceived(SshMessage msg) throws IOException {
        ...
        default: {
            ...
            printf(func,1,"Message not handled");
    ...
    protected void onServiceInit(int startMode) throws IOException {
        final String func=getFunc();
        printf(func,3,"Registering connection protocol messages");
    ...
    private void onMsgChannelClose(SshMsgChannelClose msg) throws IOException {
        ...
        printf(func,3,"Remote computer has closed channel " +
            String.valueOf(channel.getLocalChannelId()) + "[" +
            channel.getName() + "]");
    ...
    private void onMsgChannelEOF(SshMsgChannelEOF msg) throws IOException {
        ...
        try {
            printf(func,3,"Remote computer has set channel " +
                String.valueOf(msg.getRecipientChannel()) + " to EOF [" +
                channel.getName() + "]");
            channel.setRemoteEOF();
        } catch (IOException ioe) {
            printf(func,3,"Failed to close the ChannelInputStream after EOF event");
        }
    }
    ...
    private void onMsgChannelOpen(SshMsgChannelOpen msg) throws IOException {
        final String func=getFunc();
        //synchronized (activeChannels) {
            printf(func,3,"Request for " + msg.getChannelType() +
                " channel recieved");
            ...
            if (cf == null) {
                ...
                printf(func,3,"Request for channel type " + msg.getChannelType() +
                    " refused");
            ...
            try {
                printf(func,3,"Creating channel " + msg.getChannelType());

                Channel channel = cf.createChannel(msg.getChannelType(),
                        msg.getChannelData());

                // Initialize the channel
                printf(func,3,"Initiating channel");
                ...
                activeChannels.put(channelId, channel);
                printf(func,3,"Sending channel open confirmation");
    ...
    private void onMsgChannelRequest(SshMsgChannelRequest msg)
        ...
        if (channel == null) {
            printf(func,3,"Remote computer tried to make a request for " +
    ...
    private void onMsgChannelWindowAdjust(SshMsgChannelWindowAdjust msg)
        ...
        if (log.isDebugEnabled()) {
            printf(func,2,String.valueOf(msg.getBytesToAdd()) +  " bytes added to remote window");
            printf(func,2,"Remote window space is " + String.valueOf(channel.getRemoteWindow().getWindowSpace()));
        }
    }
