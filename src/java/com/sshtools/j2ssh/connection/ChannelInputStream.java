...
public class ChannelInputStream extends InputStream {
    ...
    public int available() {
        ...
        if (msgdata != null) {
            ...
            if (v.log.isDebugEnabled() && (available > 0)) {
                v.printf(func,2,String.valueOf(available) +
        ...
        if (available == 0) {
            try {
                ...
                if (v.log.isDebugEnabled()) {
                    v.printf(func,2,String.valueOf(available) +
                        " bytes of channel data available");
                }
            } catch (MessageStoreEOFException mse) {
                v.printf(func,2,"No bytes available since the MessageStore is EOF");
                available = -1;
            } catch (MessageNotAvailableException mna) {
                available = 0;
            } catch (InterruptedException ex) {
                v.printf(func,3,"peekMessage was interrupted, no data available!");
                available = 0;
            }
        }
    ...
    public void close() throws IOException {
        final String func = v.getFunc();
        v.printf(func,3,"Closing ChannelInputStream");
        messageStore.close();
    }
    ...
    private void startBlockingOperation() throws IOException {
        ...
        synchronized (lock) {
            ...
            v.printf(func,2,"Starting blocking operation");
    ...
    private void stopBlockingOperation() throws IOException {
        final String func = v.getFunc();
        synchronized (lock) {
            v.printf(func,2,"Completed blocking operation");
            blockingThread = null;
            isBlocking = false;
        }
    }
        ...
        try {
            if (type != null) {
                ...
                while ((msg == null) && !isClosed()) {
                    try {
                        v.printf(func,2,"Waiting for extended channel data");
                ...
                while ((msg == null) && !isClosed()) {
                    try {
                        v.printf(func,2,"Waiting for channel data");
