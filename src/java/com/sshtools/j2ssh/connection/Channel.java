 ...
 */
public abstract class Channel extends Version {
    ...
    protected void processChannelData(SshMsgChannelData msg) throws IOException {
        final String func=getFunc("processChannelData(SshMsgChannelData msg)");
        synchronized (state) {
            if (!isClosed()) {
                ...
                if (windowSpace < getMinimumWindowSpace()) {
                    if (log.isDebugEnabled()) {
                        printf(func,2,"Channel " + String.valueOf(localChannelId) +
                            " requires more window space [" + name + "]");
                    }

                    windowSpace = getMaximumWindowSpace() - windowSpace;
                    printf(func,2,"Requesting connection protocol increase window");
    ...
    protected void processChannelData(SshMsgChannelExtendedData msg)        throws IOException {
        final String func=getFunc("processChannelData(SshMsgChannelExtendedData msg)");
        synchronized (state) {
    ...
    public void close() throws IOException {
        final String func=getFunc("closed");
        synchronized (state) {
            if (isOpen()) {
                ...
                if (log.isDebugEnabled()) {
                     printf(func,2,"Connection is " + ((connection == null) ? "null" 
                ...
                if (remoteHasClosed ||
                     ...
                     printf(func,3,"Finializing channel close");
                    finalizeClose();
                }
            }
        }
    ...
    protected void remoteClose() throws IOException {
         final String func=getFunc("remoteClose()");
         printf(func,2,"Remote side is closing channel");
