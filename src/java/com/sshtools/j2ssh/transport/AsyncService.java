...
public abstract class AsyncService extends Service implements Runnable {
    ...
    protected void onStart() throws IOException {
        ...
        printf(func,3,"Starting " + getServiceName() + " service thread");
        thread.start();
    }
    ...
    public final void run() {
        ...
        while ((state.getValue() == ServiceState.SERVICE_STARTED) &&
            ...
            try {
                ...
                if (log.isDebugEnabled()) {
                    printf(func,2,"Routing " + msg.getMessageName());
                }

                onMessageReceived(msg);

                if (log.isDebugEnabled()) {
                    printf(func,2,"Finished processing " + msg.getMessageName());
                }
                ...
                if ((state.getValue() != ServiceState.SERVICE_STOPPED) &&
                        transport.isConnected()) {
                    printf(func,1,"Service message loop failed!", ex);
                    stop();
                }
            }
        }
        ...
        onStop();
        printf(func,3,getServiceName() + " thread is exiting");
        thread = null;
    }
