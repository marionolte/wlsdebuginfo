 ...
 * @since 0.2.0
 */
public final class SshMessageStore extends Version{
    ...
    public synchronized SshMessage peekMessage(int[] messageIdFilter, int timeout)
        throws MessageStoreEOFException, MessageNotAvailableException, InterruptedException {
        SshMessage msg;

        final String func = getFunc("peekMessage(int[] messageIdFilter, int timeout)");
        ...
        if (timeout > 0) {
            if (log.isDebugEnabled()) {
                printf(func,2,"No message so waiting for " + String.valueOf(timeout) + " milliseconds");
            }
