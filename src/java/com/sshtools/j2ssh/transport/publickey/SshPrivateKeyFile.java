 ...
 */
public class SshPrivateKeyFile extends Version {
    private static SshPrivateKeyFile o=new SshPrivateKeyFile();
    ...
    public static SshPrivateKeyFile parse(byte[] formattedKey) throws InvalidSshKeyException {
        final String func=o.getFunc("SshPrivateKeyFile parse(byte[] formattedKey)");
        if (formattedKey == null) {
            throw new InvalidSshKeyException("Key data is null");
        }

        printf(func,3,"Parsing private key file");
        ...
        if (!valid) {
           printf(func,3, "Private key is not in the default format, attempting parse with other supported formats");
