...
public class SshPublicKeyFile extends Version{
    ...
    public static SshPublicKeyFile parse(byte[] formattedKey) throws InvalidSshKeyException {
        final String func=spfkf.getFunc("SshPublicKeyFile parse(byte[] formattedKey)");
        printf(func,3,"Parsing public key file");
        ...
        if (!valid) {
            printf(func,3, "Public key is not in the default format, attempting parse with other supported formats");
