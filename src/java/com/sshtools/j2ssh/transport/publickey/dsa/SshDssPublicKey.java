...
public class SshDssPublicKey extends SshPublicKey {
    ...
    public boolean verifySignature(byte[] signature, byte[] data) throws InvalidSshKeySignatureException {
        final String func=getFunc("verifySignature(byte[] signature, byte[] data)");
        try {
            ...
            if (signature.length != 40) {
                ...
                printf(func,3,"Header is " + header);
            ...
            if ( debug > 0) {
                printf(func,3,"Verifying host key signature");
                printf(func,3,"Signature length is " + String.valueOf(signature.length));
