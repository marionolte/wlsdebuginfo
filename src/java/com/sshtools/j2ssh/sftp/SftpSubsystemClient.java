...
//    private static Log log = LogFactory.getLog(SftpSubsystemClient.class);
    ...
    public synchronized boolean initialize() throws IOException {
        final String func=getFunc();
        printf(func,3,"Initializing SFTP protocol version " + String.valueOf(version));
        ...
        if (reply instanceof SshFxpVersion) {
            result = true;
            version = ((SshFxpVersion) reply).getSSHVersion().intValue();
            printf(func,3,"Server responded with version " +
    ...
    private void getOKRequestStatus(UnsignedInteger32 requestId) throws IOException {
        ...
        try {
            if (log.isDebugEnabled()) {
                printf(func,3,"Waiting for response");
            }

            SubsystemMessage reply = messageStore.getMessage(requestId);
            printf(func,3,"Received response");
