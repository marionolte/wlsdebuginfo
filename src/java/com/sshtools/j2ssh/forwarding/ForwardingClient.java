 ...
 */
public class ForwardingClient extends Version implements ChannelFactory {
//    private static Log log = LogFactory.getLog(ForwardingClient.class);
        ...
        if (properties.getLocalForwardings().size() > 0) {
            for (Iterator it = properties.getLocalForwardings().values()
                ...
                try {
                    ...
                    printf(func,3,(("Failed to start local forwarding " + fwd) != null)
                        ? fwd.getName() : "", (Exception) ex);
                }
            }
        }
        ...
        if (properties.getRemoteForwardings().size() > 0) {
            for (Iterator it = properties.getRemoteForwardings().values()
                ...
                try {
                    ...
                    printf(func,3,(("Failed to start remote forwarding " + fwd) != null)
                        ? fwd.getName() : "", (Exception) ex);
                }
            }
        }
    ...
    public Channel createChannel(String channelType, byte[] requestData) throws InvalidChannelException {
        ...
        if (channelType.equals(ForwardingSocketChannel.X11_FORWARDING_CHANNEL)) {
            ...
            try {
                ...
                printf(func,2,"Creating socket to " +
    ...
    public void startX11Forwarding() throws IOException, ForwardingConfigurationException {
        ...
        if (log.isDebugEnabled()) {
            printf(func,3,"X11 forwarding started");
            printf(func,2,"Address to bind: " +
                x11ForwardingConfiguration.getAddressToBind());
            printf(func,2,"Port to bind: " +
                String.valueOf(x11ForwardingConfiguration.getPortToBind()));
            printf(func,2,"Host to connect: " +
                x11ForwardingConfiguration.hostToConnect);
            printf(func,2,"Port to connect: " +
                x11ForwardingConfiguration.portToConnect);
        } else {
            printf(func,3,"Request for X11 rejected.");
        }
    }
    ...
    public void startRemoteForwarding(String name)
        ...
        printf(func,3,"Remote forwarding configuration '" + name + "' started");

        if (log.isDebugEnabled()) {
            printf(func,2,"Address to bind: " + config.getAddressToBind());
            printf(func,2,"Port to bind: " +
                String.valueOf(config.getPortToBind()));
            printf(func,2,"Host to connect: " + config.hostToConnect);
            printf(func,2,"Port to connect: " + config.portToConnect);
        }
    }
    ...
    public void stopLocalForwarding(String uniqueName)
        ...
        ForwardingListener listener = (ForwardingListener) localForwardings.get(uniqueName);
        listener.stop();
        printf(func,3,"Local forwarding configuration " + uniqueName + "' stopped");
    }
    ...
    public void stopRemoteForwarding(String name)
        ...
        printf(func,3,"Remote forwarding configuration '" + name + "' stopped");
    }
