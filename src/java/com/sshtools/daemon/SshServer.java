 ...
 */
public abstract class SshServer extends Version{
 ...
 */
    public void startServer() throws IOException {
        final String func=getFunc("startServer()");
        printf(func,4,"Starting server");
        shutdown = false;
        startServerSocket();
        thread = new Thread(new RunnableT() {
                    public void run() {
                        setRunning();
                        try {
                            startCommandSocket();
                        } catch (IOException ex) {
                            printf(func,1,"Failed to start command socket", ex);
 ...
 */
    protected void startCommandSocket() throws IOException {
        final String func=getFunc("startCommandSocket()");
        try {
            ...
            while ((client = commandServerSocket.accept()) != null) {
                printf(func,2,"Command request received");
            ...
            if (!shutdown) {
                printf(func,1,"The command socket failed", e);
            }
        }
    }
 ...
 */
    public void stopServer(String msg) throws IOException {
        final String func=getFunc("stopServer(String msg)");
        printf(func,2,"Shutting down server");
        shutdown = true;
        printf(func,3,msg);
        shutdown(msg);
        listener.stop();
        printf(func,1,"Stopping command server");
        ...
        try {
            ...
            printf(func,1,"stop error "+ioe.getMessage(), ioe);
        }
    }
 ...
 */
    protected TransportProtocolServer createSession(Socket socket) throws IOException {
        final String func=getFunc("createSession(Socket socket)");
        printf(func,2,"Initializing connection");

        InetAddress address = socket.getInetAddress();

        /*( (InetSocketAddress) socket
     .getRemoteSocketAddress()).getAddress();*/
        printf(func,2,"Remote Hostname: " + address.getHostName()+"\nRemote IP: " + address.getHostAddress());
    ...
    class ConnectionListener extends RunnableT {
        ...
        public void run() {
            ...
            setRunning();
            try {
                printf(func,2,"Starting connection listener thread");
                        ...
                        public void onDisconnect(TransportProtocol transport) {
                            ...
                            if (state.getValue() != StartStopState.STOPPED) {
                                synchronized (activeConnections) {
                                    printf(func,2,transport.getUnderlyingProviderDetail() + " connection closed");
                ...
                try {
                    ...
                    if (state.getValue() != StartStopState.STOPPED) {
                        printf(func,1,"The server was shutdown unexpectedly", ex);
                    }
                }
                ...
                // Closing all connections
                 printf(func,1,"Disconnecting active sessions");
                ...
                listener = null;
                 printf(func,1,"Exiting connection listener thread");
            } catch (IOException ex) {
                 printf(func,1,"The server thread failed", ex);
            //} finally {
            //    thread = null;
            }
        ...
        //@Override
        //public void start() {
        //    thread = new SshThread(this, "SShThread Connection listener", true);
        //    thread.start();
        //}
